...
void AnimationSprite::CaculateWorldMatrix()
{
	Matrix m_Sc, m_T;
...
AnimationSprite::AnimationSprite(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, std::shared_ptr<Texture> texture, int numFrames, float delayTime)
	: BaseObject()
{
	m_pModel = model;
	m_pShader = shader;
...
AnimationSprite::AnimationSprite(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, Vector4 color)
	: BaseObject()
{
	m_pModel = model;
	m_pShader = shader;
...
void AnimationSprite::Draw()
{
	...
	iTempShaderVaribleGLID = -1;
	iTempShaderVaribleGLID = m_pShader->GetUniformLocation((char*)"u_maxFrame");
	if (iTempShaderVaribleGLID != -1)
...
void AnimationSprite::SetSize(GLint width, GLint height)
{
	m_iWidth = width;
	m_iHeight = height;
	m_Vec3Scale = Vector3((float)m_iWidth / screenWidth, (float)m_iHeight / screenHeight, 1);
